  ...
  return (
    <Switch.Group as='div' className='flex items-center'>
      <Switch.Label as='span' className='mr-3 text-sm flex justify-center gap-2 items-center'>
        <Image src='/mistral-logo.jpeg' width={25} height={25} alt='1 icon' className={`${isGPT && 'opacity-50'}`} />
        <span className={`font-medium ${isGPT ? 'text-gray-400' : 'text-gray-900'}`}>Mixtral</span>{' '}
      </Switch.Label>
      <Switch
        checked={isGPT}
        onChange={setIsGPT}
      ...
      <Switch.Label as='span' className='ml-3 text-sm flex justify-center gap-2 items-center'>
        <span className={`font-medium ${!isGPT ? 'text-gray-400' : 'text-gray-900'}`}>GPT-3.5</span>{' '}
        <Image
